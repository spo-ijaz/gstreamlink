...
namespace StreamlinkGtk.Preferences {
    ...
    public class PageStreamingProviders : PreferencesPage {
        ...
        [GtkChild]
        public unowned Adw.ComboRow combo_row_streaming_provider;
        [GtkChild]
        public unowned SignalListItemFactory combo_row_streaming_provider_factory;
        ...
        construct {
            ...
            //
            this.combo_row_streaming_provider.set_model (this.provider_plugin_controller.list_store_plugin_providers);
            ...
            for (uint position = 0; position <= this.provider_plugin_controller.list_store_plugin_providers.get_n_items (); position++) {
                ...
                if (startup_plugin_provider.id == startup_provider_id) {
                    ...
                    this.combo_row_streaming_provider.set_selected (position);
                    break;
                }
            }

            // Handle provider selection.
            this.combo_row_streaming_provider.notify.connect (this.combo_row_streaming_provider_notify_handler);
        }

        public PageStreamingProviders (ProviderPluginController provider_plugin_controller) {
            Object (provider_plugin_controller: provider_plugin_controller);
        }

        [GtkCallback]
        private void combo_row_streaming_provider_setup_handler (SignalListItemFactory factory, Object object) {
        ...
        [GtkCallback]
        private void combo_row_streaming_provider_bind_handler (SignalListItemFactory factory, Object object) {

            ListItem list_item = object as ListItem;
            if (list_item.child == null) {

                this.combo_row_streaming_provider_setup_handler (factory, list_item);
            }
        ...
        private void combo_row_streaming_provider_notify_handler (ParamSpec paramspec) {

            if (this.combo_row_streaming_provider.get_model () != null && this.combo_row_streaming_provider.get_model ().get_n_items () > 0 && paramspec.get_name () == "selected-item") {

                PluginProvider plugin_provider_selected = this.combo_row_streaming_provider.get_model ().get_item (this.combo_row_streaming_provider.get_selected ()) as PluginProvider;
